...
def generate_first_sentence_brute_force(start_word, sentence_len, topic_vector, words):
    ...
    while l != sentence_len:
        ...
        if topic_vector[words.index(w2)] < avg:
            continue
        if(((l==4) and (len(w2)==3)) or ((l==5) and (len(w2)==2)) or l==2):
            sentence.append(w2)          
            l += len(w2)         
    return sentence
...
def generate_first_sentence(start_word, sentence_len, topic_vector, words, collocations_h):
    ...
    for i in range(start_len, sentence_len):
        for j in f[i]:
            ...
            topic = topic_vector[words.index(j)]
...
if __name__ == '__main__':
    ...
    while not success:
        ...
        try:
            random.choice(collocations_v[start_word][len(start_word)])
            success = True
            break
        except:
            if success:
    ...
    print(poem[0].encode("gbk"))
    print(poem[1].encode("gbk"))
    print(poem[2].encode("gbk"))
    print(poem[3].encode("gbk"))
